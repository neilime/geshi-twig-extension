...
build-php: ## Build PHP image
	...
	@DOCKER_BUILDKIT=1 docker build -t "geshi-twig-extension-php:${PHP_VERSION}" --build-arg "VERSION=${PHP_VERSION}" --build-arg "UID=${UID}" --build-arg "GID=${GID}" .
...
define run-php
	@docker run -it --rm -v ${PWD}:${PWD} -w ${PWD} geshi-twig-extension-php:${PHP_VERSION} $(1)
endef
